...
| **17** | [#321](https://github.com/Arbor-Live/Arbor-Live-Portal/pull/321) | Artist Needed: `events/event-artist-needed.spec.ts` posts a need on the new Artists tab, has a band request to perform from `/dashboard/opportunities`, and asserts the need flips to inquiring with the message visible to staff — plus the band-only guard on that page. Moved the lineup/payout and rider panels off Overview onto the Artists tab, so `event-band-riders.spec.ts` and `band-shows-home.spec.ts` now navigate to `/artists`. |
