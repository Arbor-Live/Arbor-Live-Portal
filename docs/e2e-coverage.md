...
| **4** | [#62](https://github.com/Arbor-Live/Arbor-Live-Portal/pull/62) | Crew application triage (turn away / convert / trainee assign), crew `/onboarding` completion, band `/onboarding/artist` completion |
...
| **17** | [#321](https://github.com/Arbor-Live/Arbor-Live-Portal/pull/321) | Artist Needed + lineup: `events/event-artist-needed.spec.ts` opens a slot on the new Artists tab, has a band request to perform from `/dashboard/opportunities`, asserts the slot flips to inquiring with the message visible to staff, and asserts the band sees their set and soundcheck windows on "Your shows". Also covers the band-only guard on that page. Moved the lineup/payout and rider panels off Overview onto the Artists tab, so `event-band-riders.spec.ts` and `band-shows-home.spec.ts` now navigate to `/artists`. |
