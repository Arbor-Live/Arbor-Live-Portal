 ...
 * They may appear on invoices for host transparency, but earned-revenue metrics
 * and net profit treat them as non-margin (equipment / crew / fees are the margin).
 *
 * Net profit strips invoice pass-through from the total, then subtracts operating
 * event costs only. Matching `bandsCostUsd` / `externalRentalsCostUsd` already
 * represented by that strip are not subtracted again; cost overruns still reduce profit.
 */
