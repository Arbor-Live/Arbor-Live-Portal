            ...
            <div className="grid gap-8 md:grid-cols-2">
              ...
              <div className="flex flex-col gap-6">
                <div>
                  ...
                  <p className="mt-2 text-muted-foreground dark:text-status-zinc-300">
                ...
                {event.venueName || event.venueAddress ? (
                  <div>
                    <h2 className="text-sm font-semibold uppercase tracking-wider text-muted-foreground dark:text-status-zinc-400">
                      Venue
                    </h2>
                    <p className="mt-1">{event.venueName}</p>
                    {event.venueAddress ? (
                      <p className="text-sm text-muted-foreground dark:text-status-zinc-300">{event.venueAddress}</p>
                    ) : null}
                    {event.googleMapsUrl ? (
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="mt-1 inline-block text-sm text-status-emerald-800 underline-offset-4 hover:underline dark:text-status-emerald-400"
                      >
                        View on Google Maps
                      </a>
                    ) : null}
                ...
                {event.caption ? (
                  <div>
                    ...
                    <p className="mt-1 whitespace-pre-wrap text-base text-foreground/80 dark:text-status-zinc-200">
                      {event.caption}
                    </p>
                  </div>
                ) : null}
                  ...
                  <div className="flex flex-wrap gap-4 text-sm">
                    {event.additionalLinks.map((link) => (
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="inline-flex items-center gap-1.5 text-status-emerald-800 underline-offset-4 hover:underline dark:text-status-emerald-400"
                      >
