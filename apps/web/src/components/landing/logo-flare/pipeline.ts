...
/** Arbor mark fills its viewBox; optical center is near geometric center. */
const GLYPH_CENTER_IN_BOX: Point = [0.5, 0.5];
/**
 * Canvas is oversized vs the layout box (`CANVAS_OVERSCAN` in the mark
 * component) so the halo has room. Logo height is a fraction of the shorter
 * canvas side; for our portrait mark that side is width, so
 * `(408/307)/overscan` makes the glyph match the static icon footprint.
 */
  ...
  setFrameUniforms(
    ...
    this.effects.composite.set({
      params: {
        ...
        scatter: 1,
        rimFill: 1,
        // Soft outer fade on the oversized canvas so residual scatter
        // doesn't hard-cut at the bitmap edge (logo sits well inside).
        verticalEdgeFade: 0.12,
      },
    });
  }
